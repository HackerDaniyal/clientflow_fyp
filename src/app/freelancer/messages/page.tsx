...
export default async function FreelancerMessages() {
  const supabase = createClient();
  
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/auth/login");

  // Fetch workspaces with recent messages
  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <p className="text-sm text-text-secondary">All your workspace conversations in one place.</p>
      </header>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 text-[16px] font-semibold">
                      {workspace.client?.full_name?.charAt(0).toUpperCase() || "C"}
                    </div>
                    <div>
                      <h3 className="text-[15px] font-medium text-brand-dark">{workspace.client?.full_name || "Client"}</h3>
